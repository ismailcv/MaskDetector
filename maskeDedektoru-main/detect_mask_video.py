...
def detect_and_predict_mask(frame, faceNet, maskNet):
	...
	# yüz listemizi, bunlara karşılık gelen konumları ve yüz maskesi ağımızdaki
    #tahminlerin listesini başlatma yeri
	faces = [] 
	locs = [] 
	preds = [] 
	...
	for i in range(0, detections.shape[2]):
        ...
        # doğruluk eğer 0.5 ten fazla ise döngüye girer
		if confidence > 0.5:
        #yüzün x y kordinatları kodu
			box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
			(startX, startY, endX, endY) = box.astype("int")

			# sınırlayıcı kutuların çerçevenin boyutları 
            #dahilinde olmasını sağlayan kod
